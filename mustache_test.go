...
func TestBasic(t *testing.T) {
	for idx , test := range tests {
		...
		if output != test.expected {
			t.Fatalf("Index[%d] %q expected %q got %q", idx, test.tmpl, test.expected, output)
		}
...
var nested_map = map[string]interface{}{
	"a":map[string]interface{}{
		"b":map[string]string{
			"c":"c value"}}}

var nested = []Test{
	{`{{a.b.c}}`, nested_map, "c value"},
	{`{{#a}}{{b.c}}{{/a}}`, nested_map, "c value"},
	{`{{#a.b}}{{c}}{{/a.b}}`, nested_map, "c value"},
}
...
func TestFileInherit(t *testing.T) {
	...
	output := RenderFile(filename, context)
	if output != expected {
		t.Fatalf("TestFileInherit expected %q got %q", expected, output)
	}
}